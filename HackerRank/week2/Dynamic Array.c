...
int* dynamicArray(int n, int queries_rows, int queries_columns, int** queries, int* result_count) {
    ...
    int* lastAnswerList = malloc(queries_rows * sizeof(int));
    int lastAnswer = 0;
    int count = 0;
    ...
    for (int i = 0; i < queries_rows; i++) {
        ...
        if (type == 1) {
            seqSizes[seqIndex]++;
            seqList[seqIndex] = realloc(seqList[seqIndex], seqSizes[seqIndex] * sizeof(int));
            seqList[seqIndex][seqSizes[seqIndex] - 1] = y;
        } else if (type == 2) {
            int idx = y % seqSizes[seqIndex];
            lastAnswer = seqList[seqIndex][idx];
            lastAnswerList[count++] = lastAnswer;
        }
    }
