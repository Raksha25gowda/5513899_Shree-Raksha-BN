...
int minimumMoves(int n, char** grid, int startX, int startY, int goalX, int goalY) {
    ...
    while (front < rear) {
        ...
        for (int i = cur.x - 1; i >= 0 && grid[i][cur.y] != 'X'; i--)
            if (!visited[i][cur.y]) { visited[i][cur.y] = 1; queue[rear++] = (Node){i, cur.y, cur.dist + 1}; }
        for (int i = cur.x + 1; i < n && grid[i][cur.y] != 'X'; i++)
            if (!visited[i][cur.y]) { visited[i][cur.y] = 1; queue[rear++] = (Node){i, cur.y, cur.dist + 1}; }
        for (int j = cur.y - 1; j >= 0 && grid[cur.x][j] != 'X'; j--)
            if (!visited[cur.x][j]) { visited[cur.x][j] = 1; queue[rear++] = (Node){cur.x, j, cur.dist + 1}; }
        for (int j = cur.y + 1; j < n && grid[cur.x][j] != 'X'; j++)
            if (!visited[cur.x][j]) { visited[cur.x][j] = 1; queue[rear++] = (Node){cur.x, j, cur.dist + 1}; }
    }
    return -1;
}
