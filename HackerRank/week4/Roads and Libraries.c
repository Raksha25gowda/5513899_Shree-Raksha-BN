...
void addEdge(List *list, int val) {
    if (list->size == list->cap) {
        list->cap = list->cap ? list->cap * 2 : 4;
        list->adj = realloc(list->adj, list->cap * sizeof(int));
    }
    list->adj[list->size++] = val;
}

void bfs(int start, int n, List *graph, int *visited, int *count) {
    int queue[100000];
    int front = 0, rear = 0;
    queue[rear++] = start;
    visited[start] = 1;
...
long roadsAndLibraries(int n, int c_lib, int c_road, int cities_rows, int cities_columns, int** cities) {
    ...
    List *graph = calloc(n + 1, sizeof(List));
    int i;
    for (i = 0; i < cities_rows; i++) {
        addEdge(&graph[cities[i][0]], cities[i][1]);
        addEdge(&graph[cities[i][1]], cities[i][0]);
    }
    ...
    for (i = 1; i <= n; i++) {
        if (!visited[i]) {
            int compSize = 0;
            bfs(i, n, graph, visited, &compSize);
            totalCost += c_lib + (long)(compSize - 1) * c_road;
        }
    }

    for (i = 0; i <= n; i++) free(graph[i].adj);
    free(graph);
    free(visited);
