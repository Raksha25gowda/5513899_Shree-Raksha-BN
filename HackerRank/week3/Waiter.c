...
int* waiter(int number_count, int* number, int q, int* result_count) {
    int limit = 20000;
    char *isComposite = calloc(limit+1, 1);
    int *primes = malloc(q * sizeof(int));
    int pcount = 0;
    for (int p = 2; p <= limit && pcount < q; p++) {
        if (!isComposite[p]) {
            primes[pcount++] = p;
            if ((long long)p * p <= limit) {
                for (int k = p * p; k <= limit; k += p) isComposite[k] = 1;
            }
        }
    }
    free(isComposite);
    ...
    for (int i = 0; i < q; i++) {
        ...
        while (A_top > 0) {
            int plate = A[--A_top];
            if (plate % primes[i] == 0) B[B_top++] = plate;
            else next_A[next_A_top++] = plate;
        }

        while (B_top > 0) answers[ans_idx++] = B[--B_top];
    ...
    while (A_top > 0) answers[ans_idx++] = A[--A_top];
