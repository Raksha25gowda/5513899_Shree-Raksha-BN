...
int* climbingLeaderboard(int ranked_count, int* ranked, int player_count, int* player, int* result_count) {
    ...
    *result_count = player_count;
    ...
    // Create a distinct sorted leaderboard (descending)
    ...
    int i = distinct_count - 1;  // index on distinct leaderboard (start from lowest score)
    for (int j = 0; j < player_count; j++) {
        while (i >= 0 && player[j] >= distinct[i]) {
            i--;
        }
        result[j] = i + 2; // rank is index+2 because i is zero-based and we stepped past the position
    }
